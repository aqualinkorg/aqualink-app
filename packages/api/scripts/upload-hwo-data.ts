...
async function run() {
  ...
  const dataUploadsRepository = dataSource.getRepository(DataUploads);
  const dataUploadsSitesRepository = dataSource.getRepository(DataUploadsSites);
  const sourcesRepository = dataSource.getRepository(Sources);
  const timeSeriesRepository = dataSource.getRepository(TimeSeries);
  ...
  try {
    ...
    await refreshMaterializedView(dataUploadsRepository);
  } finally {
