...
export class addDeployedStatus1610596421453 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "reef_status_enum" AS ENUM('in_review', 'rejected', 'approved', 'shipped', 'deployed')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "reef_status_enum_old" AS ENUM('in_review', 'rejected', 'approved', 'shipped')`,
    );
    await queryRunner.query(
      `ALTER TABLE "reef" ALTER COLUMN "status" DROP DEFAULT`,
    );
