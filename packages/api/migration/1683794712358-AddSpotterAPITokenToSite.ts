...
export class AddSpotterAPITokenToSite1683794712358 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "site" ADD "spotter_api_token" character varying`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."site_audit_column_name_enum" AS ENUM('name', 'sensor_id', 'status', 'video_stream', 'max_monthly_mean', 'display', 'spotter_api_token')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      ...
      BEGIN
          ...
          IF (NEW.spotter_api_token is null and OLD.spotter_api_token is not null)
            OR (NEW.spotter_api_token is not null and OLD.spotter_api_token is null)
            OR (NEW.spotter_api_token != OLD.spotter_api_token) 
          THEN
          INSERT INTO "site_audit" (old_value, new_value, site_id, column_name)
          VALUES (OLD.spotter_api_token, NEW.spotter_api_token, NEW.id, 'spotter_api_token');  
          END IF;
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "site" DROP COLUMN "spotter_api_token"`,
    );
  }
}
