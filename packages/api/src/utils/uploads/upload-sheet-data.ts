...
//
//   if (!dateKey) {
//     return new BadRequestException(`Date Time column not found`);
//   }
//
...
const sourceItems: Record<SourceType, Record<string, SourceItem>> = {
  ...
  [SourceType.METLOG]: {
    '#': { ignore: true },
    'Date Time, GMT-04:00': { time: 'Timestamp', ignore: true },
...
//
//   if (!isDate(date)) {
//     throw new BadRequestException('Wrong excel date input');
//   }
//
//   return date;
// };
...
export const uploadTimeSeriesData = async (
  ...
  if (Object.keys(sourceItems).includes(sourceType)) {
    const workSheetsFromFile = xlsx.parse(filePath, { raw: true });
    const workSheetData = workSheetsFromFile[0]?.data;
    // const headers = Object.keys(sourceItems[sourceType]);
    ...
    const results = preResult.map((item) => {
      ...
      const timestamp = Array.isArray(timestampIndex)
        ? new Date(`${item[timestampIndex[0]]} ${item[timestampIndex[1]]}`)
        : new Date(item[timestampIndex]);
