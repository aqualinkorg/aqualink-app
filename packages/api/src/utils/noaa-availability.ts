...
class NOAAAvailability {
  ...
  private BFS(
    ...
    if (!head) return null;
    if (visited.has(`${head.lon},${head.lat}`)) return this.BFS(visited, stack);
    if (this.worldMap[head.lon][head.lat] === false)
  ...
  async getNearestAvailablePoint(
    ...
    const result = this.BFS(visited, stack);
...
export default NOAAAvailability;
